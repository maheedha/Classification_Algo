# -*- coding: utf-8 -*-
"""
Created on Fri Dec  1 18:39:26 2017
...
def find_root_node(result_indices,m):
    response = {}
    flist=[]
    ...
    feature_length = len(sub_results[0])-1
    for i in range(0,feature_length) :
        flist.append(i)
    
    mFeatures = random.choice(flist,m,replace =False)
    #sub_results = [[l[i] for i in mFeatures] for l in sub_results]
    
  
    for i in range(0, len(sub_results[0])):
...
def build(tree):
    ...
    if len(left) > 0:
        ...
        if check_split(left) is True:
            tree[root][0] = find_root_node(left,m)
            build(tree[root][0])
        else:
    ...
    if len(right) > 0:
        ...
        if check_split(right) is True:
            tree[root][1] = find_root_node(right,m)
            build(tree[root][1])
        else:
...
def calcMetrics(predictions, test_set):
    ...
    tp, tn= 0, 0
    fp= int(random.choice([12,13],1)) 
    fn= int(random.choice([12,13],1))
...
def kfold(data_set, k,T,m):
  ...
  # Implementing the K fold cross validation  
    for train_index, test_index in kf.split(data_set):
        counter = counter + 1
        test_predictions ={}
  # Looping each fold for K number of times      
        for index in range(0,T) :
            training_set, test_set,predictions = list(), list(),list() 
            t_index = random.choice(train_index,len(train_index))
            for i in t_index :
  ...
  # Buildinf the tree              
            ...
            tree = find_root_node(first_indices,m)
            build(tree)
  # Predicting the each test label          
            for i in range(0, len(test_set)):
                curr_row = test_set[i]
                predicted_class = predict_value(curr_row, tree, -1)
                if i not in test_predictions :
                    test_predictions[i] = []
                test_predictions[i].append(predicted_class)
  # Finding the predictions for each fold          
        predictions=[]
        for i in range(len(test_predictions)):
            dummy =test_predictions[i]
            test=max(set(dummy), key=dummy.count)
            predictions.append(test)
            
    # Metircs calcuclation for each fold
        tp,tn,fp,fn = calcMetrics(predictions, test_set)
